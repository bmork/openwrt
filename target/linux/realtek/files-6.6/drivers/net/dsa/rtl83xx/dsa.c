			    ...
			    struct netlink_ext_ack *extack)
{
	 ...
	 * PVID=1 from initial setup in place and ignore all subsequent settings.
	 */
	if (port != priv->cpu_port) {
			    ...
			    const struct switchdev_obj_port_vlan *vlan)
{
	...
	/* Reset to default if removing the current PVID from non CPU ports */
	if (vlan->vid == pvid && port != priv->cpu_port)
		rtl83xx_vlan_set_pvid(priv, port, 0);
